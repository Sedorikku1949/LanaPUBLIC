...
function getDifference(nb) {
    if (nb <= 1000) return "[ss]s"; if (nb <= 1000) return "[ss]s"; if (nb <= 60000) return "[mm]m et [ss]s"; if (nb <= 3600000) return "[hh]h, [mm]m et [ss]s"; else return "[DD] jours, [hh]h, [mm]m et [ss]s";
}
...
function getDuration(date1) {
    ...
    return format.replace(/\[(DD|hh|mm|ss)\]/g, (_, m) => {
        switch (m) { case "DD": return day; case "hh": return hours; case "mm": return ("0" + minutes).slice(-2); case "ss": return ("0" + seconds).slice(-2) }
    })
}
