...
async function interval(ms) {
  database.Client.interval.push(setInterval(async function(){
    // database save
    if (getDate(Date.now(), "[hh]:[mm]") == "23:59" && getDate(Date.now(), "[DD]_[MM]_[YYYY]") !== lastSave) {
      ...
      client.channels.cache.get("899047568326164540")?.send({
